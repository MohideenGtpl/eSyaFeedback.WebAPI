...
namespace eSyaFeedback.DL.Repository
{
    public class PatientFeedbackTransactionRepository: IPatientFeedbackTransactionRepository
    {
        ...
        public async Task<List<DO_PatientFeedback>> GetPatientFeedbackHeader(string feedbacktype,DateTime fromdate,DateTime todate)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    if (feedbacktype == "All")
                    {
                        //var fa = await db.GtFbptdt.Where(x => x.DocumentDate >= fromdate.Date && x.DocumentDate <= fromdate.AddDays(28).Date)
                      var fa = await db.GtFbptdt.Where(x => x.DocumentDate >= fromdate.Date && x.DocumentDate <= todate.Date)
                     .Select(r => new DO_PatientFeedback
                     {
                         BusinessKey = r.BusinessKey,
                         FinancialYear = r.FinancialYear,
                         DocumentID = r.DocumentId,
                         DocumentNumber = r.DocumentNumber,
                         FBDocumentKey = r.FbdocumentKey,
                         ...
                         CommentOnServices = r.CommentOnServices,
                     }).ToListAsync();
                        return fa;
                    }

                    else {
                    var fa = await db.GtFbptdt.Where(x => x.FeedBackType.ToUpper().Replace(" ", "") == feedbacktype.ToUpper().Replace(" ", "")
                    && x.DocumentDate >= fromdate.Date && x.DocumentDate <= todate.Date)
                     .Select(r => new DO_PatientFeedback
                     {
                         BusinessKey = r.BusinessKey,
                         FinancialYear = r.FinancialYear,
        ...
        public async Task<DO_PatientFeedback> GetPatientFeedbackDetails(int businesskey, long fbdocumentkey,string feedbacktype)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    ...
                    var fa = await db.GtFbptdt.Where(m => m.FbdocumentKey == fbdocumentkey && m.FeedBackType.ToUpper().Replace(" ", "") == feedbacktype.ToUpper().Replace(" ", ""))
                     .Select(r => new DO_PatientFeedback
                     {
                         BusinessKey = r.BusinessKey,
                         FinancialYear = r.FinancialYear,
                         ...
                         l_QuestionnaireMaster = db.GtFbptqe.Where(h => h.FbdocumentKey == r.FbdocumentKey && h.BusinessKey == businesskey)
                         .Join(db.GtFbqems,
                         x => x.QuestionnaireId,
                         y => y.QuestionnaireId,
                        (x, y) => new DO_QuestionnaireMaster
                        {
                            ...
                            FbdocumentKey = x.FbdocumentKey,
                            QuestionnaireValue = x.QuestionnaireValue,
                            Comments = x.Comments,
                            QuestionnaireDesc = y.QuestionnaireDesc
                        }).GroupBy(e => e.QuestionnaireID).Select(g => g.First()).ToList()
                     }).FirstOrDefaultAsync();
